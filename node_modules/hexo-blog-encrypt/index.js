...
const log = hexo.log;
//const log = require('hexo-log') 顯示debug log, 是否要改成這樣?

const defaultConfig = {
  'abstract': '札記系列文章需要密碼才可觀看，但是hexo-blog-encrpt似乎跟pjax不相容，所以沒有密碼輸入畫面，目前沒有能力解決。',
...
// filter 過濾器: 過濾器用於修改特定資料，Hexo 將資料依序傳給過濾器，而過濾器可以針對資料進行修改，這個概念是從 WordPress 借來的。
...
// generator 生成器
//https://hexo.io/zh-cn/api/generator
// generator建立路由 => hexo-blog-encrypt 布局映射到箭頭函數

hexo.extend.generator.register('hexo-blog-encrypt', () => [
  {
    //當前路徑 + lib/heb.styke.css
...
//fs读取文件的时候，是将文件一次性读取到本地内存。而如果读取一个大文件，一次性读取会占用大量内存，效率很低，这个时候需要用流来读取。流是将数据分割段，一段一段的读取，可以控制速率,效率很高,不会占用太大的内存。
//https://juejin.cn/post/6844903633788600333
